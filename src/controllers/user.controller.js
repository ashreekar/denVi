...
const getUserChannelProfile = asyncHandler(async (req, res) => {
    ...
    // writting aggregation pipeline which takes array of pipelines
    const channel = await User.aggregate([
        {
            // pipeline to match channel username
            $match: {
                username: username?.toLowerCase() //for safety
            }
        },
        {
            //now lookup of channel
            $lookup: {
                from: "subscriptions",
                localField: "_id",
        ...
        {//npw lookup for our subrcriptions user
            $lookup: {
                from: "subscriptions",
                localField: "_id",
        ...
        {
            // gives total count of subrcribers and user subscription
            $addFields: {
                subscriberscount: {
                    $size: "$subricribers"
                },
                ...
                isSubscribed: {
                    $condition: { // whether this user is part of subscribers field subscriber
                        if: { $in: [req.user?._id, "$subricribers.subscriber"] },
                        then: true,
                        else: false
                    }
                }
        ...
        { // gives selected values like fullName
            $project: {
                fullName: 1,
                username: 1,
                subscriberscount: 1,
...
const getWatchHistory = asyncHandler(async (req, res) => {
    const user = await User.aggregate([
        ...
        },
        {
            $lookup: {
                ...
                as: "watchHistory",
                pipeline: [
                    {   // sub pipeline for owner details
                        $lookup: {
                            from: "users",
                            localField: "owner",
                            foreignField: "_id",
    ...
    return res.status(200).json(new ApiError(200, user[0].watchhistory, "watch history fetched sucessfully"))
})

export { registerUser, loginUser, logoutUser, refreshAcceasToken, changeCurrentPassword, getCurrentUser, updateAccountDetails, updateAvatar, updateCoverImage, getUserChannelProfile, getWatchHistory };